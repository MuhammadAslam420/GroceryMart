            ...
            <div class="col">
              <div class="product-item">
                ...
                <div class="flex justify-content-between">
                  <span class="qty">1 Unit</span><span class="flex rating p-1"><i
                      class="bi bi-star-fill text-yellow-400"></i> 4.5</span>
                </div>
                <span class="price">{{$product->price}}</span>
                <div class="flex align-items-center ">
                  <span class="input-group-btn">
                    <button class="quantity-left-minus btn w-10 bg-red-500" wire:click='decreaseQty({{ $product->id }})' data-type="minus"
                      data-field="">
                  ...
                  <input type="text" class="form-control text-dark h-7 w-30 border rounded-lg"
                  wire:model.live="qty.{{ $product->id }}" min="0" max="500">
                  <span class="input-group-btn">
                    <button class="quantity-right-plus btn w-10 bg-green-500" wire:click='increaseQty({{ $product->id }})' data-type="plus"
                      data-field="">
                      <svg width="18" height="16">
                ...
                <a href="#"
                  wire:click.prevent="storeProduct('{{ $product->id }}','{{ $product->name }}',{{ $product->price }})"
                  class="nav-link font-bold text-teal-600 p-1 hover:text-yellow-500"><i
                    class="bi bi-bag-check-fill"></i> Add to Cart </a>
              </div>
            </div>
            @empty
            <div class="col">
